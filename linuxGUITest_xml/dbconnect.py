    ...
    try:
        ...
        for file in cu.fetchall():
            ...
            if result != 'Success':
                ...
                createlog(name="createHTML", debug=['Result', result])
            trTemp1 << td(file[-1]) + td(file[0]) + td(file[1]) + td(file[2]) + td(file[3])+td('null')+td(result)
        fp = dbTable+'DetailPage.html'
        page.printOut(file=fp)
        reportdir = './report'
        if not os.path.exists(reportdir):
			os.mkdir('report')
        shutil.copy(fp,reportdir)
        os.remove(fp)
    except Exception as error_Message:
...
def createHtml(reportname = 'function',filename='TestResultPage.html',*hinfo):
    ...
    page << p1+p2+p3
    page << p('Summary : ', style='color:red;', align='center') << a('Detail', href=reportname+'DetailPage.html')
